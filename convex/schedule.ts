...
export const listAccessibleLiveClasses = query({
  ...
  handler: async (ctx) => {
    const user = await getCurrentUserFromAuth(ctx);
    if (!user) return [];
    const [schedules, studentSchoolIds, studentCampusId] = await Promise.all([
      ctx.db
        .query("classSchedule")
    ...
    const liveSchedules = schedules.filter(
      (schedule) => schedule.sessionType === "live",
    );
    const results = await Promise.all(
...
export const markLive = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const classData = await assertCanAdministerLiveSchedule(
      ctx,
      user._id,
      schedule,
    );
