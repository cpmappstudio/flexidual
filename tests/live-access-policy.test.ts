...
test("enrolled students always retain access", () => {
  assert.equal(
    canStudentAccessLiveClass({ isEnrolled: true, studentSchoolIds }),
    true,
  );
});
...
test("school access requires an allowed grade in the same school", () => {
  assert.equal(
    canStudentAccessLiveClass({
      isEnrolled: false,
      liveAccess: schoolAccess,
      studentGrade: "07",
      classSchoolId: "school-a",
      studentSchoolIds,
    }),
    true,
  );
...
test("course access defaults to private and normalizes selected grades", () => {
  assert.deepEqual(normalizeLiveAccess(), {
    mode: "private",
    allowedGradeCodes: [],
  });
  assert.deepEqual(
