...
import { MAX_BULK_LESSONS, parseBulkLessons } from "@/lib/bulk-lessons";
...
export function LessonDialog({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditing && lesson) {
        ...
        const parsedBulkLessons = parseBulkLessons(bulkLessonsText);
        if (parsedBulkLessons.invalidLines.length > 0) {
          toast.error(
            t("lesson.bulk.invalidLines", {
              lines: parsedBulkLessons.invalidLines.join(", "),
            }),
          );
          return;
        }
        ...
        finalQueue.push(
          ...parsedBulkLessons.lessons.map((item, index) => ({
            id: `bulk-${index}`,
            title: item.title,
        ...
        if (finalQueue.length > MAX_BULK_LESSONS) {
  ...
  const dialogDesc = isEditing
    ? t("lesson.editDescription")
    : t("lesson.addDescription");

  // Calculate button label
  const parsedBulkLessons = parseBulkLessons(bulkLessonsText);
  const pendingLessonCount =
    queue.length + parsedBulkLessons.lessons.length + (formData.title ? 1 : 0);
  const createLabel =
    pendingLessonCount > 0
      ? t("common.createAllNumber", { count: pendingLessonCount })
      : formData.title
  ...
  return (
    ...
    >
      <div className="space-y-6">
          ...
          <div className="grid gap-2 p-4 bg-muted/30 rounded-lg border">
            ...
            <div>
              <Select
                ...
                onValueChange={(val) =>
                  setFormData({ ...formData, curriculumId: val })
                }
              >
        ...
        <div
          className={`space-y-4 ${!isEditing ? "p-4 border rounded-lg bg-card shadow-sm" : ""}`}
        >
              ...
              <Badge variant="outline" className="text-xs">
                {defaultCurriculumId
                  ...
                  : formData.curriculumId
                    ? t("lesson.selectedCurriculum")
                    : t("lesson.pendingSelection")}
              </Badge>
            )}
          </div>
          ...
          <div className="grid gap-2">
            <Label>
              {t("lesson.title")} <span className="text-destructive">*</span>
            </Label>
            <Input
        ...
        {!isEditing && (
          <div className="space-y-4">
            ...
            <BulkLessonsInput
              ...
              onValueChange={setBulkLessonsText}
            />

            <div className="space-y-2 border-t pt-4">
              <div className="flex items-center justify-between px-1">
                <Label className="text-muted-foreground">
                  {t("lesson.queue", { count: queue.length })}
                </Label>
                {queue.length > 0 && (
                  <Button
                    variant="ghost"
                ...
                {queue.length === 0 ? (
                  ...
                  <div className="divide-y divide-border/50">
                    {queue.map((q, i) => (
                      <div
                        key={q.id}
                        className="flex items-center justify-between p-3 text-sm bg-card/50 hover:bg-card transition-colors"
                      >
