...
export function CompanionClassroomUI({ roomName }: { roomName: string }) {
  ...
  // Use screen.height/width (not window.inner*) — stable across virtual keyboard events.
  useEffect(() => {
  ...
  const startBroadcast = async () => {
    ...
    // Ghost canvas attached to document.body at near-zero opacity.
    // iOS Safari keeps the MediaStream alive as long as the canvas is in the DOM.
    ...
    const paintFrame = async () => {
      ...
      try {
        ...
        // Keep last good frame on failure
      }
    };

    await paintFrame();

    // Event-driven: re-paint only when shapes actually change
