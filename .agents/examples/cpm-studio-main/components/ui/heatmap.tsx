...
function getHtmlProps(props: HeatmapProps): HTMLAttributes<HTMLDivElement> {
  const htmlProps = { ...props } as Record<string, unknown>;

  for (const key of heatmapPropKeys) {
...
function isDateString(day: string | null | undefined): day is string {
...
function getAllDays(start: string, end: string): string[] {
  ...
  const days: string[] = [];
  const curr = createLocalDate(start);
...
function padToWeekStart(days: string[]): (string | null)[] {
  ...
  const firstDay = createLocalDate(days[0]!).getDay();
...
function getMonthLabel(week: (string | null)[]) {
  ...
  const lastDay = [...week].reverse().find(isDateString);
  return !lastDay
    ? null
    : createLocalDate(lastDay).toLocaleString("default", {
        month: "short",
      });
}
...
export default function Heatmap(props: HeatmapProps) {
  ...
  const monthLabels = weeks.map((week, i) => {
    ...
    const prevLabel = prevWeek ? getMonthLabel(prevWeek) : null;
    return label !== prevLabel ? label : null;
  });

  const fontSize = Math.min(16, cellSize);

  const safeProps = getHtmlProps(props);
  ...
  const getCellColor = (value: number) => {
    if (colorMode === "interpolate") {
      ...
      const index = map(value, maxValue, colorArray.length);

      return colorArray[index] ?? colorArray[0] ?? "transparent";
    }
  };
  ...
  return (
    ...
    >
      ...
      {weeks.map((week, i) => {
        const weekAnchorDay = week.find(isDateString);
      ...
      <TooltipProvider>
        {weeks.map((week, weekIdx) =>
          week.map((day, dayIdx) => {
            ...
            const dateForDisplay = createLocalDate(day);
              ...
              <Tooltip key={day}>
                <TooltipTrigger asChild>
                  <ValueIndicator
                    ...
                    aria-label={
                      cellLabelFunction
                        ? cellLabelFunction(dateForDisplay, safeValue)
                        : `${day}: ${safeValue} event${safeValue !== 1 ? "s" : ""}`
                    }
                    id={`heatmap-cell-${day}`}
                    cellSize={cellSize}
                ...
                <TooltipContent>
                  {tooltipContentFunction ? (
                    tooltipContentFunction(dateForDisplay, safeValue)
                  ) : (
